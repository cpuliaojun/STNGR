...
"""
# parse arguments
input_file = './../data/nps/raw_nps_smiles.txt'
output_file = './../data/nps/raw_nps_smiles_clean.txt'
...
# vocabulary = Vocabulary(vocab_file='./../data/voc.txt',smi_max_len=90,scaf_max_len=80)
...
def clean(smi):
    ...
    ## define valid symbols
    valid = set(['Br', 'C', 'Cl', 'F', 'H', 'I', 'N', 'O', 'P', 'S'])
    mols = [mols[idx] for idx, atoms in enumerate(elements) if len(set(atoms) - valid) == 0]
    print("parsed " + str(len(mols)) + " molecules with all valid atoms (C/N/O/P/S/F/Br/Cl/I)")
...
def cat():
    ...
    input.columns = ['raw']
    input['clean'] = input['raw'].apply(lambda x:clean(x))
    input.to_csv('./../data/nps/raw_nps_smiles_clean.csv',index=False)
